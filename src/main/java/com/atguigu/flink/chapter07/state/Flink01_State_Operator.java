...
/**
 * 算子状态---列表状态(同一个应用挂抛异常后,因checkpoint会自动重启恢复数据)
 * <p>
 * 主要用于Source和Sink中
 * <p>
 * 注意：匿名内部类只能实现一个接口或某一个类,不能实现多个
 *
 * @author Evan
...
public class Flink01_State_Operator {
    ...
    public static class MyFlatMapFuntion implements FlatMapFunction<String, String>, CheckpointedFunction {
        ...
        public void initializeState(FunctionInitializationContext ctx) throws Exception {
            ...
            // ListStateDescriptor列表描述器
            wordsState = ctx.getOperatorStateStore().getListState(new ListStateDescriptor<String>("ws", String.class));
